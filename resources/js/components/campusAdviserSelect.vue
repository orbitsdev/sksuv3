...
<template>
  ...
  >
    <option v-for="item in items" :key="item.id" :value="item.id" > {{ item.user.first_name }} - {{ item.user.last_name }} | {{  item.campus.name}} |   {{ item.school_year.from }}  - {{ item.school_year.to  }} </option>
  </select>
