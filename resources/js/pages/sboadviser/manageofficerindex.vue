...
function saveCampus() {
  form.post(route("officers.create"), {
    preserveState: true,
    onSuccess: () => {
...
function updateCampus() {
  ...
  form.post(route("officers.update"), {
    preserveState: true,
    onSuccess: () => {
    form.id = null;
      show_form.value = false;
...
async function deleteSelected(){
  ...
  try {
    const response = await router.post(route('officers.deleteselected'), {
...
<template>
  <adminlayout>
    ...
    >
     ...
     <Link :href="route('officers.index')" class="w-40 inline-flex  justify-center rounded-md  px-4 py-2 font-medium shadow-sm text-sm text-gray-700 hover:bg-gray-50 border border-gray-300 bg-white h-10 mr-2">
  ...
  <path fill-rule="evenodd" d="M9.53 2.47a.75.75 0 010 1.06L4.81 8.25H15a6.75 6.75 0 010 13.5h-3a.75.75 0 010-1.5h3a5.25 5.25 0 100-10.5H4.81l4.72 4.72a.75.75 0 11-1.06 1.06l-6-6a.75.75 0 010-1.06l6-6a.75.75 0 011.06 0z" clip-rule="evenodd" />
      ...
      :headers="['', 'Name', 'Position' ,'Campus Adviser', 'Campus' , ]">
        <tr
          class="divide-x divide-gray-200"
          v-for="item in props.officers.data"
          :key="item"
        >
          ...
          <Tcell class="uppercase"> {{ item.user.first_name }} {{ item.user.last_name }}</Tcell>
          <Tcell class="capitalize"> {{ item.position }} </Tcell>
       <Tcell class="capitalize" > {{ item.campus_adviser != null  ?  item.campus_adviser.user.first_name +' ' +item.campus_adviser.user.last_name  : 'None'}} </Tcell>
       <Tcell class="capitalize" > {{ item.campus_adviser.campus != null  ?  item.campus_adviser.campus.name : 'None'}} </Tcell>
        ...
        <div class="mt-6 py-4 bg-white" v-if="$props.officers.links.length > 0">
          <Pagination
            v-if="$props.officers.data.length > 0"
            class="block"
            :links="$props.officers.links"
          />
        </div>
        </div>
    ...
    class="block w-full max-w-lg h-10 px-2 capitalize rounded-md border shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm"
  >
  ...
  <div v-else class="p-2 rounded  bg-rose-600 animate-pulse  text-white"> Empty</div>
    <p class="text-red-700 text-sm" v-if="$page.props.errors.position">
          ...
          <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
            ...
            <div class="mt-2">
              <p class="text-sm text-gray-500">
                Are you sure you want to delete officers ? All of your data will be
